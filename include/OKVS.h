...
public:
    ...
    vector<block> encode(const vector<block> &keys, const vector<block> &values, u64 numThreads = 0);

    void encode(const vector<block> &keys, const vector<block> &values, vector<block> &encoding, u64 numThreads = 0);
